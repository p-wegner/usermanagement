...
class TenantService(
    ...
    fun createTenant(tenantCreateDto: TenantCreateDto): GroupDto {
        ...
        val existingGroups = keycloakGroupFacade.getGroups(tenantGroupName)
        ...
        val clientRoles = roleService.getRoles()
    ...
    fun getTenants(): List<GroupDto> {
        return keycloakGroupFacade.getGroups()
            .filter { it.name.startsWith(TENANT_PREFIX) }
            .map { groupService.getGroup(it.id) }
    }
